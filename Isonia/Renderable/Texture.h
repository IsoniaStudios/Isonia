...
{
	Texture(Device& device, const std::string& textureFilepath) : device{ device }
	{
		...
		: device{ device }, format{ format }, extent{ extent }
	{
		...
		if (usage & VK_IMAGE_USAGE_SAMPLED_BIT)
		{
			...
			descriptor.sampler = mTextureSampler;
			descriptor.imageView = mTextureImageView;
	...
	~Texture()
	{
		vkDestroySampler(mDevice.device(), textureSampler, nullptr);
		vkDestroyImageView(mDevice.device(), textureImageView, nullptr);
		vkDestroyImage(mDevice.device(), textureImage, nullptr);
		vkFreeMemory(mDevice.device(), textureImageMemory, nullptr);
	}
	...
	static Texture* CreateTextureFromFile(LveDevice& device, const std::string& filepath)
	{
	...
	void UpdateDescriptor()
	{
		descriptor.sampler = mTextureSampler;
		descriptor.imageView = mTextureImageView;
		descriptor.imageLayout = mTextureLayout;
	}
